...
fn parse_gltf(path: &Path) -> Result<Vec<Meshes>> {
    ...
    vertices.push(Vertex3D { x: position[0], y: position[1], z: position[2]});

meshes_res.push(Meshes { vertices, indices, texture_id: Default::default(), scissor: Scissor { width: 0, height: 0, x: 0, y: 0 } });        }
    }}
    Ok(meshes)
}
